<template>
	<div>
		<v-card :class="roleID == 2 && 'mt-2 mb-2 pa-1'" :outlined="roleID == 2" elevation="0">
			<h1 class="subheading grey--text">Panel Content Mall</h1>
			<v-card>
				<v-tabs
					v-model="tab"
					fixed-tabs
							...
							<div class="px-1">
								...
								>
									...
									<template #[`item.mall`]="{ item }">
										<span v-html="item.data_MT.nama_mall" />
									</template>
    ...
    >
      <v-card>
        <v-toolbar
          ...
          color="light-blue darken-3"
        >
          <v-toolbar-title>{{editedIndex == 0 ? 'Tambah' : 'Ubah'}} Data Content Mall</v-toolbar-title>
          <v-spacer />
          <v-toolbar-items>
            <v-btn
              icon
    ...
    >
      <v-card>
        <v-toolbar
          ...
          color="light-blue darken-3"
        >
          <v-toolbar-title>View Lampiran Data Content Mall</v-toolbar-title>
          <v-spacer />
          <v-toolbar-items>
            <v-btn
              icon
...
export default {
  name: 'ContentMall',
	...
	metaInfo: {
		title: "E-Mall (Content Mall) - WIN.ID",
		htmlAttrs: {
			lang: "id",
			amp: true,
		},
	...
	watch:{
		...
		inputContent: {
			deep: true,
			handler(value){
				if(value.id_mall == null){ this.inputContent.id_mall = '' }
				...
				if(this.editedIndex == 0 && !this.inputContent.UnixText) { this.getMall(2, value.id_mall) }

				if(value.id_mall != '' && value.id_kategori_content != '' && value.judul_content != '' && value.link != '' && value.deskripsi != '' && 
					value.filefoto != ''){
	...
	methods: {
      ...
      uploadFiles: "upload/uploadFiles",
		...
		getContent(code) {
			...
			let payload = {
				method: "get",
				url: `moduleMain/getAllContent?table=ContentMall&id_kategori_content=${code}`,
		...
		getMall(index, value = null) {
			...
			.then((res) => {
				...
				if(index == 1) {
					this.MallOptions = dataKumpul
				}else{
					dataKumpul.map(val => {
						if(val.id_mall == value) {
		...
		bukaDialog(item, index){
			...
			if(index == 0){
				...
				this.inputContent.id_content_mall = item.id_content_mall ? item.id_content_mall : ''
				this.inputContent.id_mall = item.id_mall ? item.id_mall : ''
      ...
      let bodyData = {
        jenis: index == 0 ? 'ADD' : 'EDIT',
				content: 'Mall',
        id_content_mall: index == 0 ? '' : this.inputContent.id_content_mall,
        id_mall: this.inputContent.id_mall,
		...
		async uploadLampiran(index, dataUpload) {
			if(dataUpload){
				const bodyData = {
					...
					id: index == 0 ? null : this.inputContent.id_content_mall,
					judul_content: this.inputContent.judul_content,
					nama_folder: this.inputContent.UnixText,
					nama_file: `ContentMall-${this.convertDate(new Date().toISOString().slice(0,10))}${this.makeRandom(8)}`,
					jenis: "images",
					bagian: "mall",
					table: "t_content_mall",
					files: dataUpload,
				};
				try {
    ...
    HapusRecord(item) {
      let bodyData = {
        jenis: 'DELETE',
				content: 'Mall',
        id_content_mall: item.id_content_mall,
    ...
    crop() {
      ...
      this.DialogCropContent = false
		...
		clearForm() {
			...
			this.inputContent.id_content_mall = ''
			this.inputContent.id_mall = ''
