				...
				<v-row no-gutters>
					<v-col cols="12" md="9">
						...
						label="Pencarian..."
							...
							@keyup.enter="getLogLoginPeserta(1, limit, searchData)"
						/>
					</v-col>
					<v-col cols="12" md="3" class="pl-2 d-flex justify-end align-center">
						<v-autocomplete
							...
							dense
							hide-details
							:disabled="DataLogLoginPeserta.length ? false : true"
						/>
					</v-col>
				</v-row>
			</v-col>
			...
			<v-col cols="2" class="mt-2 text-right">
				<div class="d-flex justify-end">
					...
					<v-icon
						...
						large
						:disabled="DataLogLoginPeserta.length ? pageSummary.page != 1 ? false : true : true"
					...
					<v-icon
						style="cursor: pointer;"
						large
						:disabled="DataLogLoginPeserta.length ? pageSummary.page != pageSummary.totalPages ? false : true : true"
						@click="() => { page = pageSummary.page + 1 }"
					>
  ...
  metaInfo: {
	...
	watch: {
		page: {
			deep: true,
			handler(value) {
				this.getLogLoginPeserta(value, this.limit, this.searchData)
			}
		},
    limit: {
			deep: true,
			handler(value) {
				this.getLogLoginPeserta(1, value, this.searchData)
			}
		},
    searchData: {
			deep: true,
			handler(value) {
        if (value == null) {
          this.getLogLoginPeserta(1, this.limit, this.searchData)
        }
			}
		},
    sortDesc: {
	...
	methods: {
		...
		getLogLoginPeserta(page = 1, limit, keyword) {
      ...
      this.pageSummary = {
			...
			this.DataLogLoginPeserta = []
			this.pageOptions = [{ value: 1 }]
			this.isLoading = true
			let payload = {
				method: "get",
				url: `settings/getLoggerPeserta?page=${page}&limit=${limit}${keyword ? `&keyword=${keyword}` : ''}&sort=${this.kumpulSort}`,
				authToken: localStorage.getItem('user_token')
			};
			this.fetchData(payload)
			.then((res) => {
				let resdata = res.data.result;
        this.DataLogLoginPeserta = resdata.records
        this.pageSummary = {
