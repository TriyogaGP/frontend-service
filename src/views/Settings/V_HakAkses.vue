...
export default {
  ...
  data: () => ({
    ...
    pageOptions: [
      ...
      { text: "Nama Role", value: "namaRole", sortable: true },
  ...
  metaInfo: {
	...
	watch: {
		page: {
			deep: true,
			handler(value) {
				this.getHakAkses(value, this.limit, this.searchData)
			}
		},
    limit: {
			deep: true,
	...
	methods: {
      ...
      this.pageSummary = {
			...
			let payload = {
				method: "get",
				url: `settings/getRole?page=${page}&limit=${limit}${keyword ? `&keyword=${keyword}` : ''}&sort=${this.kumpulSort}`,
