				...
				<v-row no-gutters>
					<v-col cols="12" md="9">
						...
						label="Pencarian..."
							...
							@keyup.enter="getMetodePembayaran(1, limit, searchData)"
						/>
					</v-col>
					<v-col cols="12" md="3" class="pl-2 d-flex justify-end align-center">
						<v-autocomplete
							...
							dense
							hide-details
							:disabled="DataMetodePembayaran.length ? false : true"
						/>
					</v-col>
				</v-row>
			</v-col>
			...
			<v-col cols="2" class="mt-2 text-right">
				<div class="d-flex justify-end">
					...
					<v-icon
						...
						large
						:disabled="DataMetodePembayaran.length ? pageSummary.page != 1 ? false : true : true"
					...
					<v-icon
						style="cursor: pointer;"
						large
						:disabled="DataMetodePembayaran.length ? pageSummary.page != pageSummary.totalPages ? false : true : true"
						@click="() => { page = pageSummary.page + 1 }"
					>
...
export default {
  ...
  data: () => ({
    ...
    pageOptions: [
      ...
      { text: "Kode Bank", value: "kodeBank", sortable: true },
      { text: "Nama Bank Display", value: "namaBankDisplay", sortable: true },
      { text: "Kode Bank Product", value: "kodeBankProduct", sortable: true },
      { text: "Kategori", value: "kategori", sortable: true },
      { text: "Pajak Bank", value: "pajakBank", sortable: true },
  ...
  metaInfo: {
	...
	watch: {
		page: {
			deep: true,
			handler(value) {
				this.getMetodePembayaran(value, this.limit, this.searchData)
			}
		},
    limit: {
			deep: true,
			handler(value) {
				this.getMetodePembayaran(1, value, this.searchData)
			}
		},
    searchData: {
			deep: true,
			handler(value) {
        if (value == null) {
          this.getMetodePembayaran(1, this.limit, this.searchData)
        }
			}
		},
    sortDesc: {
	...
	methods: {
		...
		getMetodePembayaran(page = 1, limit, keyword) {
      ...
      this.pageSummary = {
			...
			this.DataMetodePembayaran = []
			this.pageOptions = [{ value: 1 }]
			this.isLoading = true
			let payload = {
				method: "get",
				url: `settings/getPayment?page=${page}&limit=${limit}${keyword ? `&keyword=${keyword}` : ''}&sort=${this.kumpulSort}`,
				authToken: localStorage.getItem('user_token')
			};
			this.fetchData(payload)
			.then((res) => {
				let resdata = res.data.result;
        this.DataMetodePembayaran = resdata.records
        this.pageSummary = {
