				...
				<v-row no-gutters>
					<v-col cols="12" md="9">
						...
						label="Pencarian..."
							...
							@keyup.enter="getKurir(1, limit, searchData)"
						/>
					</v-col>
					<v-col cols="12" md="3" class="pl-2 d-flex justify-end align-center">
						<v-autocomplete
							...
							dense
							hide-details
							:disabled="DataKurir.length ? false : true"
						/>
					</v-col>
				</v-row>
			</v-col>
			...
			<v-col cols="2" class="mt-2 text-right">
				<div class="d-flex justify-end">
					...
					<v-icon
						...
						large
						:disabled="DataKurir.length ? pageSummary.page != 1 ? false : true : true"
					...
					<v-icon
						style="cursor: pointer;"
						large
						:disabled="DataKurir.length ? pageSummary.page != pageSummary.totalPages ? false : true : true"
						@click="() => { page = pageSummary.page + 1 }"
					>
...
export default {
  ...
  data: () => ({
    ...
    pageOptions: [
      ...
      { text: "Nama Kurir", value: "namaKurir", sortable: true },
  ...
  metaInfo: {
	...
	watch: {
		page: {
			deep: true,
			handler(value) {
				this.getKurir(value, this.limit, this.searchData)
			}
		},
    limit: {
			deep: true,
			handler(value) {
				this.getKurir(1, value, this.searchData)
			}
		},
    searchData: {
			deep: true,
			handler(value) {
        if (value == null) {
          this.getKurir(1, this.limit, this.searchData)
        }
			}
		},
    sortDesc: {
	...
	methods: {
      ...
      this.pageSummary = {
			...
			let payload = {
				method: "get",
				url: `settings/getKurir?page=${page}&limit=${limit}${keyword ? `&keyword=${keyword}` : ''}&sort=${this.kumpulSort}`,
				authToken: localStorage.getItem('user_token')
			};
			this.fetchData(payload)
			.then((res) => {
				let resdata = res.data.result;
        this.DataKurir = resdata.records
        this.pageSummary = {
